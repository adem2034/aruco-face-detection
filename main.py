...
##############################
# Dictionary to map ArUco marker IDs to registered IDs
aruco_id_mapping = {
    # Add your mapping here, e.g., ArUco ID: Registered ID
}
########################################################################""""
def detect_aruco_markers(frame):
    ...
    if ids is not None:
        ...
        for i, id in enumerate(ids):
            name_data = df[df['ID'] == id[0]]['NAME']
            centroid = corners[i][0].mean(axis=0).astype(int)

            if name_data.empty or pd.isnull(name_data.values[0]):
                cv2.putText(frame, "UNKNOWN", (centroid[0], centroid[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            else:
                name = name_data.values[0]
                cv2.putText(frame, name, (centroid[0], centroid[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
###################################
        ...
        # # Associate recognized ArUco markers with registered individuals
        #             recognized_ids = [aruco_id_mapping.get(aruco_id, "Unknown") for aruco_id in aruco_ids]

        # # Print recognized IDs (for testing, you can replace this with further processing)
        #             print("Recognized IDs:", recognized_ids)
        
        # # Your existing face detection code goes here...
        # else:
        #                 print("No ArUco markers detected in the frame.")
#####################################################
